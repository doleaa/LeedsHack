...
class BaseMessage:
    ...
    @classmethod
    def unpack_string(cls, d):
        l, d = cls.unpack_uint(d)
        return d[0:l], d[l:]

    @staticmethod
    def unpack_pos_off(d, pos):
        return struct.unpack("<Q", d[pos:pos + 8])[0], pos + 8

    @staticmethod
    def unpack_pos_uint(d, pos):
        return struct.unpack("<I", d[pos:pos + 4])[0], pos + 4

    @classmethod
    def unpack_pos_string(cls, data, position):
        lenstring, position = cls.unpack_pos_uint(data, position)
        string = data[position:position + lenstring]
        return string, position + lenstring

    @classmethod
    def unpack_cipher(cls, d):
        l, d = cls.unpack_uint(d)
        if (l % 16) != 0:
            l_c = ((l / 16) + 1) * 16
        else:
            l_c = l
        return cls.cipher.decipher(d[0:l_c])[:l], d[l_c:]

    @classmethod
    def pack_cipher(cls, s):
        return cls.pack_uint(len(s)) + cls.cipher.cipher(s)
...
class Ping(BaseMessage):
    ...
    def make(self):
        return self.pack_uint(self.code) + self.pack_uint(self.msg_id)

    def parse(self, data):
        self.msg_id, data = self.unpack_uint(data)
        return self
...
class Login(BaseMessage):
    ...
    def make(self):
        return self.pack_uint(self.code) + \
               pack_string(self.algorithm) + \
               pack_string(self.chresponse) + \
...
class UserShares(BaseMessage):
    ...
    def parse(self, data):
        ...
        for i in range(dirs):
            user_dir, pos = self.unpack_pos_string(data, pos)

            self.shares[user_dir] = {}
            ...
            for j in range(files):
                ...
                self.shares[user_dir][filename] = [size, extension, attributes]
        return self
...
class JoinRoom(BaseMessage):
    ...
    def make(self):
        if self.private:
            ...
            private = chr(0)
        return self.pack_uint(self.code) + pack_string(self.room) + private
...
class RoomTickers(BaseMessage):
    ...
    def parse(self, data):
        self.room, data = self.unpack_string(data)
        self.tickers = {}
        num_tickers, data = self.unpack_uint(data)
...
class MessageUsers(BaseMessage):
    ...
    def make(self):
        user_list = ""
        for u in self.users:
            user_list += pack_string(u)
        return self.pack_uint(self.code) + self.pack_uint(self.users.len()) + user_list + pack_string(self.message)
...
class PrivateRoomToggle(BaseMessage):
    ...
    def make(self):
        if self.enabled:
            ...
            enabled = chr(0)
        return self.pack_uint(self.code) + self.pack_uint(self.enabled) + enabled
...
class PrivateRoomList(BaseMessage):
    ...
    def parse(self, data):
        ...
        for i in range(n):
            ...
            num_users, data = self.unpack_uint(data)
            status, data = self.unpack_uint(data)
            self.rooms[room] = (num_users, status)
        return self
...
class Search(BaseMessage):
    ...
    def __init__(self, msg_type=None, query=None):
        super().__init__()
        self.type = msg_type
        self.query = query
        self.ticket = None
...
class TransferState(BaseMessage):
    ...
    def unpack_transfer(self, data):
        ...
        file_pos, data = self.unpack_off(data)
        file_size, data = self.unpack_off(data)
        rate, data = self.unpack_uint(data)
        return Transfer(is_upload, user, path, state, error, file_pos, file_size, rate, place), data
...
class TransferRemove(BaseMessage):
    ...
    def __init__(self, upload=None, user=None, path=None, transfer=None):
        super().__init__()
        self.upload = upload
        self.user = user
        self.path = path
    ...
    def make(self):
        if self.upload:
            ...
            upload = chr(0)
        return self.pack_uint(self.code) + upload + pack_string(self.user) + pack_string(self.path)
...
class DownloadFolderTo(BaseMessage):
    ...
    def make(self):
        return self.pack_uint(self.code) + \
               pack_string(self.user) + \
               pack_string(self.path) + \
               pack_string(self.download_path)
